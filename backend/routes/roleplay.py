...
routes/roleplay.py
──────────────────
Property pitch roleplay feature — Carol persona.

No HTTP endpoints here. This module exposes three functions that
routes/chat.py calls via _state injection:

    get_roleplay_session(conversation_id)       -> dict | None
    upsert_roleplay_session(conversation_id, n) -> None
    run_roleplay(conversation_id, user_msg, history) -> str

Trigger phrases (detected in routes/chat.py):
    "practice pitch", "role play property", "roleplay property", "start practice"

End phrases (detected inside run_roleplay):
    "end practice", "end role play", "end roleplay", "stop practice"

DB table required (created in main.py init_db):
    roleplay_sessions (conversation_id TEXT PK, exchanges INTEGER, created_at TEXT)
"""

import httpx
from routes.database import get_roleplay_session, upsert_roleplay_session, delete_roleplay_session
from routes.config import OLLAMA_URL, MODEL_FAST

# ── Carol's character prompt ──────────────────────────────────────────────────
...
def _build_feedback_prompt(exchanges: int, history: list) -> str:
    history_text = "\n".join(
...
async def run_roleplay(conversation_id: str, user_msg: str, history: list) -> str:
    ...
    if any(phrase in msg for phrase in END_PHRASES):
        ...
        feedback = res.json()["message"]["content"]
